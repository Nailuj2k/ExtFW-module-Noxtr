...
## 1.4.241 — Logout web y Nostr unificados

- `/login/logout` elimina también la sesión Nostr aunque Noxtr no esté cargado en esa
  página, e impide que IndexedDB, NIP-46 o NIP-07 la restauren al volver.
- El cierre se propaga a las demás pestañas y limpia identidad, sockets, followers,
  contactos, mensajes y trades sensibles que siguieran en memoria.
- Volver con el historial del navegador ya no recupera una vista Nostr desde bfcache.
- Si el servidor detecta que la sesión ExtFW ha caducado al cargar los trades, ejecuta
  el mismo cierre Nostr además de mostrar el aviso.

## 1.4.240 — Backups identificados por tipo de identidad

- Los backups nuevos declaran su tipo de identidad (`nsec`, `nip46`, `nip07` o
  `npub`) y si contienen una clave privada capaz de restaurar el acceso.
- Al exportar desde NIP-46 se avisa de que el signer nunca entrega la `nsec` y de que
  el archivo conserva datos, pero no permite recuperar el acceso ni la conexión remota.
- Al cargar uno de esos backups se muestra un mensaje específico en lugar de tratarlo
  como un archivo defectuoso. No se exportan secretos ni credenciales del canal NIP-46.

## 1.4.239 — Un solo flujo de acceso NostrConnect

- Eliminado de Noxtr el flujo de acceso NIP-46 que generaba su propio URI, QR y
  autenticación web, incluido el modal y el enlace de reconexión que lo abría.
- Noxtr conserva exclusivamente el cliente NIP-46 necesario para restaurar la conexión
  creada por el módulo `login` y para solicitar firmas, cifrados y descifrados al signer.

## 1.4.238 — Acceso NostrConnect centralizado en login

- Retirado el botón NostrConnect propio de Noxtr y su manejador para evitar dos flujos
  de inicio de sesión distintos. El acceso y la salida de la cuenta quedan a cargo del
  módulo `login`.
- Se conserva el cliente NIP-46 interno que Noxtr necesita para firmar y para reconectar
  un firmador dentro de una sesión ya existente.
